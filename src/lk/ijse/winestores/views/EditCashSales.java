 ...
 */
public class EditCashSales extends javax.swing.JPanel implements CashTendered, FocusHandler, Extension {
     ...
     */
    public EditCashSales(int orderId) {
        initComponents();
        ...
        tblSearchItems.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent e) {
                ...
                try {
                    ...
                    Logger.getLogger(EditCashSales.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void fillData(int orderId) {
        ...
        try {
            ...
            customOrder = ctrlQuery.getCashOrder(orderId);
            orderItemDetails = ctrlQuery.getCashOrderItemDetails(orderId);
            ArrayList<OrderEmptyBottleDetailsDTO> emptyBottleDetails = ctrlQuery.getEmptyBottleDetails(orderId);
            if (emptyBottleDetails != null){
                orderEmptyBottleDetails = emptyBottleDetails;
            }
            chequeDetails = ctrlQuery.getChequeDetails(orderId);

            // (1) Adding order item details to the table
            for (OrderItemDetailsDTO orderItemDetail : orderItemDetails) {

                BigDecimal total = new BigDecimal(orderItemDetail.getQty());
                total = total.multiply(BigDecimal.valueOf(orderItemDetail.getSellingPrice()));
                total = total.setScale(2, RoundingMode.HALF_UP);

                Object[] rowData = {false,
                    orderItemDetail.getItemCode(),
                    ctrlQuery.getItemNameByItemCode(orderItemDetail.getItemCode()),
                    orderItemDetail.getQty() + "",
                    formatPrice(orderItemDetail.getSellingPrice()),
                    total.toPlainString()
                };
                dtmItems.addRow(rowData);
            }
            
            // (2) Adding Empty Bottle Details
            cmbEmptyBottleItemStateChanged(null);
            txtEmptyBottleQtyKeyReleased(null);
            
            // (3) Filling Cheque Details
            if (chequeDetails != null){
                chkChequeDetails.setSelected(true);
                chkChequeDetailsActionPerformed(null);
                txtChequeNumber.setText(chequeDetails.getChequeNumber());
                txtBank.setText(chequeDetails.getBank());
                txtBranch.setText(chequeDetails.getBranch());
            }
    ...
    private void txtQtyKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtQtyKeyReleased
        ...
        if (qty <= stock && qty > 0) {
            if (qty < 10) {
                ...
                getToolkit().beep();
                ImageIcon icon = new ImageIcon(EditCashSales.class.getResource("/lk/ijse/winestores/icons/error_icon.png"));
    ...
    public void processOrder(BigDecimal cashTendered) {
        ...
        customOrder = new CustomOrderDTO(customOrder.getOrderId(),
                formatDate(new Date()),
                "Admin",
...
//        }
        ...
        try {
            boolean success = controller.updateCashSale(customOrder, orderItemDetails, orderEmptyBottleDetails, chequeDetails);
            if (success) {
                // Resetting
//                resetTextFields(null);
//                dtmSearchItems.setRowCount(0);
...
//                enablePay();
                ...
                m.resetMenu(Main.MenuItems.VIEWS);
...
//                        icon);
                ...
                ImageIcon icon = new ImageIcon(this.getClass().getResource("/lk/ijse/winestores/icons/error_icon.png"));
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(this),
                        "Sorry, this order can not be edited right now due to some unexpected reason. Please try again.",
                        "Editing Failed",
                        JOptionPane.INFORMATION_MESSAGE,
                        icon);
            }
